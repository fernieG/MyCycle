...
async function createEncryptedBackup(passphrase){
  ...
  const payload={app:'My Cycle',schemaVersion:3,exportedAt:new Date().toISOString(),state:clone(state),predictions:serializablePrediction()};
...
function parseCSV(text){
  const rows=[];let row=[],field='',quoted=false;for(let i=0;i<text.length;i++){const c=text[i],n=text[i+1];if(quoted){if(c==='"'&&n==='"'){field+='"';i++}else if(c==='"')quoted=false;else field+=c}else{if(c==='"')quoted=true;else if(c===','){row.push(field);field=''}else if(c==='\n'){row.push(field);rows.push(row);row=[];field=''}else if(c!=='\r')field+=c}}if(field.length||row.length){row.push(field);rows.push(row)}return rows.filter(r=>r.some(v=>String(v).trim()!==''));
}
function stateFromCSV(text){
  const rows=parseCSV(text);if(rows.length<2)throw new Error('The CSV is empty.');const headers=rows[0].map(h=>String(h).trim().toLowerCase());const ix=name=>headers.indexOf(name);if(ix('date')<0)throw new Error('This CSV needs a date column.');
  const imported={version:3,profile:{...state.profile},periods:[],symptoms:{},flows:{},notes:{},lastBackup:null,created:todayISO()};
  for(const r of rows.slice(1)){const get=n=>{const i=ix(n);return i>=0?String(r[i]??'').trim():''},date=get('date');if(!/^\d{4}-\d{2}-\d{2}$/.test(date))continue;const type=(get('type')||'daily').toLowerCase(),end=get('end_date');if(type==='period'&&!imported.periods.some(p=>p.start===date))imported.periods.push({id:uid(),start:date,end:/^\d{4}-\d{2}-\d{2}$/.test(end)?end:null,source:'CSV import'});const flow=get('flow');if(flow)imported.flows[date]=flow;const symptomsCell=get('symptoms');if(symptomsCell){for(const item of symptomsCell.split('|')){const cut=item.lastIndexOf(':'),label=cut>=0?item.slice(0,cut):item,level=cut>=0?item.slice(cut+1):'Mild',id=symptomIdFromLabel(label);imported.symptoms[date]=imported.symptoms[date]||{};imported.symptoms[date][id]=severityFromText(level)}}const note=get('note');if(note)imported.notes[date]=note;}
  validatePeriodCollection(imported.periods);return imported;
}
function openCSVRestore(text,fileName){const imported=stateFromCSV(text);showRestoreChoices({state:imported,exportedAt:null,sourceName:fileName||'CSV import'});}
...
function showRestoreChoices(payload){
  ...
  const source=payload.sourceName?`Source: ${escapeHtml(payload.sourceName)}.`:`Backup created ${escapeHtml(payload.exportedAt?new Date(payload.exportedAt).toLocaleString():'on an unknown date')}.`;
  openSheet(`<h2>Restore data</h2><p>${source}</p><div class="group"><div class="row"><span>Cycles</span><strong>${imported.periods.length}</strong></div><div class="row"><span>Date range</span><strong>${dateRange}</strong></div><div class="row"><span>Daily records</span><strong>${new Set([...Object.keys(imported.symptoms),...Object.keys(imported.flows),...Object.keys(imported.notes)]).size}</strong></div></div><div class="status-note"><strong>Merge</strong> keeps current data, skips duplicate cycles and refuses overlapping imported cycles. <strong>Replace</strong> removes current local data and restores the imported data.</div><div class="action-stack"><button class="primary" id="mergeRestoreBtn">Merge without duplicates</button><button class="secondary danger" id="replaceRestoreBtn">Replace local data</button></div>`);
  ...
  document.getElementById('replaceRestoreBtn').onclick=()=>{if(confirm('Replace all current local data with this import?')){applyRestore(pendingRestore,'replace');closeSheet()}};
}
...
document.getElementById('importInput').onchange=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const text=String(reader.result||''),isCSV=file.name.toLowerCase().endsWith('.csv')||file.type==='text/csv';if(isCSV){openCSVRestore(text,file.name);return}const parsed=JSON.parse(text);if(parsed.format===BACKUP_FORMAT)openRestorePasswordSheet(parsed);else if(parsed.periods&&parsed.profile){if(confirm('This is an older unencrypted prototype backup. Continue to review restore choices?'))showRestoreChoices({state:parsed,exportedAt:parsed.exportedAt})}else throw new Error('Unsupported file format.')}catch(err){alert(err.message||'This file could not be read. Use a My Cycle .csv, .mycycle or JSON backup.')}};reader.readAsText(file);e.target.value=''};
